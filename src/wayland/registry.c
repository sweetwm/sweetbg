...
bool sweetbg_registry_init(
	...
	if (reg->registry == NULL) {
		fprintf(stderr,
			"sweetbgd: failed to get the wayland registry\n");
		return false;
	}
	...
	if (reg->compositor == NULL) {
		fprintf(stderr,
			"sweetbgd: compositor does not expose wl_compositor\n");
		ok = false;
	}
	if (reg->shm == NULL) {
		fprintf(stderr,
			"sweetbgd: compositor does not expose wl_shm\n");
		ok = false;
	}
	if (reg->layer_shell == NULL) {
		fprintf(stderr, "sweetbgd: compositor lacks wlr-layer-shell "
				"(zwlr_layer_shell_v1)\n");
		ok = false;
	}
